...
import com.example.helloworld.api.GrammarCreator;
import com.example.helloworld.api.GrammarDataFetcher;
import com.example.helloworld.api.GrammarGetByName;
...
public class HelloWorldApplication
    ...
    public static GrammarDao grammarDao;
    ...
    private static RuntimeWiring buildWiring(HelloWorldConfiguration configuration) {
        ...
        final GrammarDataFetcher gFetcher = new GrammarDataFetcher();
        final GrammarGetByName byNameFetcher = new GrammarGetByName();
        final GrammarCreator creator = new GrammarCreator();

        final RuntimeWiring wiring = RuntimeWiring.newRuntimeWiring()
                .type("Query",
                        typeWiring -> typeWiring.dataFetcher("saying", fetcher))
                .type("Query", typeWiring -> typeWiring.dataFetcher("listOfGrammars", gFetcher))
                .type("Query", typeWiring -> typeWiring.dataFetcher("getGrammarByName", byNameFetcher))
                .type("Mutation", typeWiring -> typeWiring.dataFetcher("createGrammar", creator))
                .build();
